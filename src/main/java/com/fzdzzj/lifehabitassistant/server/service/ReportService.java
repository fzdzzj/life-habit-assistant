...
public class ReportService {
    ...
    private ReportDtos.ReportResponse build(String type, LocalDate start, LocalDate end) {
        ...
        List<AnalysisDtos.DailyTrend> days = r.stream().map(x -> new AnalysisDtos.DailyTrend(x.getRecordDate(), round(x.sleepMinutes() / 60d), x.getDietScore(), x.getExerciseMinutes(), x.getWaterMl(), achieved(x))).toList();
    ...
    private List<ReportDtos.WeekSummary> weekly(List<HabitRecord> records) {
        return records.stream().collect(Collectors.groupingBy(r -> r.getRecordDate().with(DayOfWeek.MONDAY), TreeMap::new, Collectors.toList())).entrySet().stream().map(e -> {
            var r = e.getValue();
            return new ReportDtos.WeekSummary(e.getKey(), round(r.stream().mapToLong(HabitRecord::sleepMinutes).average().orElse(0) / 60d), r.stream().mapToInt(HabitRecord::getExerciseMinutes).sum(), round(r.stream().mapToInt(HabitRecord::getWaterMl).average().orElse(0)));
        }).toList();
    }
