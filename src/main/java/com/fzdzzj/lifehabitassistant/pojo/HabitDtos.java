...
public final class HabitDtos {
    ...
    public record HabitRequest(@NotNull @PastOrPresent(message = "recordDate 不得晚于今天") LocalDate recordDate,
                               @Min(value = 1, message = "dietScore 不得小于 1") @Max(value = 5, message = "dietScore 不得超过 5") int dietScore,
                               @Min(value = 0, message = "exerciseMinutes 不得小于 0") @Max(value = 600, message = "exerciseMinutes 不得超过 600") int exerciseMinutes,
    ...
    public record HabitResponse(LocalDate recordDate, long sleepMinutes,
