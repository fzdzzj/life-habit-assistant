...
### Requirement: 第三方身份绑定扩展预留
WHEN 未来接入微信、QQ 等第三方登录,
系统 SHALL 通过独立第三方身份映射表关联第三方身份与用户，保持 `users` 主表、令牌与会话模型不变；本期不实现第三方登录功能、不创建空表。

#### Scenario: 预留边界
GIVEN 本演进批次仅交付后端能力且微信/QQ 登录尚未实施
WHEN 检查数据库结构
THEN 不存在第三方身份映射空表
AND `users` 表结构未因第三方登录而变更

#### Scenario: 独立身份映射
GIVEN 未来实施微信或 QQ 登录
WHEN 用户首次通过某第三方身份登录
THEN 系统在独立映射表创建第三方身份与用户绑定
AND 同一第三方身份仅能绑定一个用户
AND 一个用户可绑定多个第三方身份

#### Scenario: 无密码账号
GIVEN 用户仅通过第三方登录注册且从未设置密码
WHEN 系统创建该用户
THEN 用户无需提供密码即可完成后续第三方登录
AND 既有密码列保持兼容（随机不可登录密码或可空策略由实现选择）
AND 不影响既有密码登录用户

#### Scenario: 会话复用
GIVEN 用户已通过第三方登录建立账号
WHEN 用户后续在任意设备登录
THEN 系统复用现有会话模型签发 access token 与 refresh token
AND 多端会话、登出与撤销语义与密码登录一致
