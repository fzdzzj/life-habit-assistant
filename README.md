...
- 好友/动态/社区/排行榜、定位轨迹、用机时长等能力本期不实现，但领域边界与架构约束已固化（独立表/独立模块、排行榜复用统一统计口径、端类型仅作会话元数据），未来接入无需重构既有聚合根与迁移。详见 [扩展性预留](docs/extensibility-groundwork.md)。
...
启用方式：在 `.env` 中设置 `AI_ADVICE_ENABLED=true`，并填写 `OPENAI_API_KEY` 与 `OPENAI_MODEL`（模型 ID 以 OpenAI 官方文档为准）；配额默认每天 3 次、每月 30 次，可用 `AI_ADVICE_DAILY_LIMIT`、`AI_ADVICE_MONTHLY_LIMIT` 调整。模型输出通过结构化输出转换器严格解析为既定 JSON schema，字段缺失或解析失败自动回落到规则建议；同周期（analysis 最近 N 天/周报/月报）结果默认缓存 10 分钟（`AI_ADVICE_CACHE_TTL`、`AI_ADVICE_CACHE_MAX_SIZE` 可调），缓存只保存 AI 成功结果，命中时响应 `cached=true` 且不重复调用模型、不扣配额、不新增历史；请求带 `refresh=true` 可强制重新生成。习惯/饮料/运动/睡眠/目标数据写入后缓存立即失效。详见 [AI 建议模块设计](docs/ai-advice.md)。
