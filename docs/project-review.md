...
技术栈一句话：Spring Boot 3.5 + Java 21 + Spring Data JPA + Spring Security/JWT + MySQL + Flyway + POI/OpenPDF + Spring AI 1.1 + H2（测试），按 Issue → 分支 → 测试 → PR 的流程开发。
...
周报/月报每次请求现算：查记录 → 统一统计 → 规则建议。不建 report 表存快照。原因：数据每天都在变，快照要么过期要么维护同步逻辑；即时计算在单人数据量下完全够快，也少一个一致性难题。为减少重复查看的成本，加了按“用户 + 周期”的内存 TTL 缓存，并且习惯/明细/目标/AI 解读写入后主动失效，所以缓存只省计算、不引入过期数据；大区间导出则落到 `export_tasks` 任务表异步生成，避免在请求线程里做重活。
...
- 依赖升级：升级 Boot 到 4.x + Spring AI 2.0（接口与手动装配边界已预留，需独立任务回归 JPA 与安全）。
